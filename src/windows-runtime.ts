...
import { windowsDiscoveryAndVerificationScript } from "./windows-verification.ts";
import type { JsonObject } from "./tools.ts";

const execFileAsync = promisify(execFile);
const installationSchema = z.object({ resources: z.string(), localAppData: z.string(), discoveryMs: z.number().nonnegative(), authenticodeMs: z.number().nonnegative() });
...
export interface WindowsRuntime {
	...
	powerShellTimingsMs: { discovery: number; authenticode: number };
}
...
export async function inspectWindowsRuntime(): Promise<{ runtime?: WindowsRuntime; status: JsonObject }> {
	...
	const identity = { adapterVersion: PACKAGE_VERSION, runtimeSource: import.meta.url, runtimeImplementation: createHash("sha256").update(resolveWindowsRuntime.toString() + powershell.toString() + verifySkyDirectory.toString() + windowsDiscoveryAndVerificationScript.toString()).digest("hex").slice(0, 16), connectionReady: null };
	try {
		const runtime = await resolveWindowsRuntime();
		return { runtime, status: { ...identity, verificationMs: Math.round(performance.now() - started), platform: "win32", permissionMode: "no-permissions", runtimeVerified: true, brokerVersion: runtime.brokerVersion, timingsMs: runtime.timingsMs, powerShellTimingsMs: runtime.powerShellTimingsMs, pluginVersion: runtime.version, appMustRemainOpen: true, methods: [...WINDOWS_COMPUTER_USE_METHODS] } };
	} catch (error) {
...
export async function resolveWindowsRuntime(): Promise<WindowsRuntime> {
	...
	// This reads signatures only; no executable runs until layout and Sky verification also succeed.
	const executables = [codex, node, repl, path.join(sky, "bin", "windows", "codex-computer-use.exe")];
	const installation = installationSchema.parse(JSON.parse(await powershell(windowsDiscoveryAndVerificationScript(executables))));
	mark("discoveryAndAuthenticode");
	...
	mark("brokerVersion");
	return { codex, node, repl, moduleDir, nativePipe, localAppData: installation.localAppData, version, brokerVersion, timingsMs, powerShellTimingsMs: { discovery: installation.discoveryMs, authenticode: installation.authenticodeMs } };
}
